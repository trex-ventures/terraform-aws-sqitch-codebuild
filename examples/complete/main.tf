module "codebuild_sqitch" {
  ...
  vpc_subnet_app_ids     = "${data.terraform_remote_state.vpc_lab_production.subnet_app_ids}"
  codebuild_sqitch_sg_id = "${data.terraform_remote_state.codebuild_sqitch_shared_resources.codebuild_security_group_id}"
  codebuild_role_arn     = "${data.terraform_remote_state.codebuild_sqitch_shared_resources.codebuild_role_arn}"
